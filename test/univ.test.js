...
eq("교과 없이 과목 '실용 경제' → 사회로 추론", A('', '실용 경제'), '사회');
eq("교과 없이 과목 '경제 수학' → 수학으로 추론", A('', '경제 수학'), '수학');
eq('교과 있으면 guess=false', U.areaOf('국어', '문학').guess, false);
eq('교과 없으면 guess=true', U.areaOf('', '문학').guess, true);
eq('모르는 교과는 과목명 추론 + guess=true', U.areaOf('창의적체험', '문학').guess, true);
